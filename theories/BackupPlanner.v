...
Fixpoint prepare_blocks (fuel : nat) (afb : afblocks) (fpos : N) (fsz : N) : afblocks :=
  ...
  | S f' =>
    ...
    then
        ...
        then prepare_blocks f' {| anum := anum afb; afree := afree afb - maxsz; ablocks := {| fbanum := anum afb; fbfpos := fpos; fbsz := maxsz |} :: ablocks afb |}
        ...
        else prepare_blocks f' {| anum := anum afb + 1; afree := asz; ablocks := {| fbanum := anum afb; fbfpos := fpos; fbsz := afree afb |} :: ablocks afb |}
...
Definition analyse_file (* (nchunks : positive) *) (afree_p : N) (anum_p : positive) (fn : string) : afblocks :=
  ...
  let fuel : nat := N.to_nat (((fsize fi) / max_block_size) + 2) in
